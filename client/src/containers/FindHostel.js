...
function FindHostel() {
  ...
  useEffect(() => {
    setTextResult(
      `Tìm thấy ${
        result?.filter((item) => item.category === CTPT).length
      } bài đăng cho thuê phòng trọ ${
  ...
  const handleFindResult = () => {
    const newResult = posts
      .filter((item) => item.category === CTPT)
      .filter(
        (item) =>
      ...
      {isShowDetail ? (
        ...
        <>
          ...
          <div className=" grid grid-cols-2 gap-x-5 gap-y-5 max-w-[90%] mx-auto mt-[20px] mb-[20px] min-h-screen">
            {result?.filter((item) => item.category === CTPT).length > 0 &&
              result
                ?.filter((item) => item.category === CTPT)
                .map((item) => (
                  <div
